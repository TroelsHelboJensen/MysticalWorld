...
function drawSword(ox, oy, dir, frameIndex) {
  const cx = TILE / 2;
  const cy = 17; // anchor near hands (slightly below center)
  const angleDeg = SWORD_SWING[dir][frameIndex];
  const a = angleDeg * Math.PI / 180;

  // Handle — brown, 3 px long
  for (let d = 3; d <= 6; d++) {
...
function drawFrame(col, row, color, dir, frameIndex, type) {
  ...
  // Draw direction-aware sword on sword-type frames
  if (type === 'sword') {
