...
describe('CombatCalculator', () => {
  ...
  describe('calcDamageWithRandom', () => {
    ...
    it('crit minimum is also 1 (floor(1 * 1.5) = 1)', () => {
      const { damage, isCrit } = CombatCalculator.calcDamageWithRandom({ attack: 2 }, { defense: 20 }, 0.05);
      expect(damage).toBe(1);
      expect(isCrit).toBe(true);
    });

    it('crit threshold is exactly 0.1 — random 0.1 is NOT a crit', () => {
  ...
  describe('calcDamage', () => {
    it('returns an object with damage >= 1 and a boolean isCrit', () => {
    ...
    it('always returns damage >= 1 regardless of stat values', () => {
      for (let i = 0; i < 20; i++) {
